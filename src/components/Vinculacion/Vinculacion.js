...
export default class Vinculacion extends Component {
  ...
  async setProfile() {
    if (this.props.parent === "ver") {
      ...
      if (prof === "Básico")
        await this.setState({ profile: "2", disableInvitado: "disabled" });
      if (prof === "Intermedio") {
        await this.setState({
          profile: "3",
          disableInvitado: "disabled",
          disableBasico: "disabled",
        });
      }
      if (prof === "Avanzado") {
        await this.setState({
  ...
  async validateAddress(value, element_id) {
    ...
    if (value.length > 200) {
      ...
      error =
        "Este campo puede tener únicamente letras, espacios y los siguientes caracteres: . , - #";
    }
    $(element_id).text(error);
    if (error !== "") {
      $(element_id).show();
  ...
  async validateFields(student, isResident) {
    ...
    if (isResident)
      await this.validateEmpty(student.id_district, "#personDistrictError");
  ...
  handleSubmit = async () => {
    ...
    switch (this.state.showMyComponent) {
      ...
      case "14":
        ...
        this.createMedioAvanzado(
          currentAvanzado,
          currentAvanzadoAcademic,
          type
        );
        break;
      case "2":
            ...
            <div className="col-md-5">
              <div className="form-group required">
                ...
                >
                  ...
                  <option value="1" disabled={this.state.disableInvitado}>
                    Invitado
                  </option>
                  <option value="2" disabled={this.state.disableBasico}>
                    Básico
                  </option>
                  <option value="3" disabled={this.state.disableMedio}>
                    Medio
                  </option>
                  <option value="4" disabled={this.state.disableAvanzado}>
                    Avanzado
                  </option>
                </select>
              </div>
